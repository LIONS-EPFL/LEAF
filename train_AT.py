...
from utils_AT import train_one_epoch, evaluate
...
def main(args):
    ...
    if args.name is None:
        ...
        args.name = '-'.join([
            ...
            #f"j_{args.workers}",
            #f"p_{args.precision}",
            f"eps_{int(args.eps_adv*255)}",
            f"k_{args.k_adv}",
            f"contr_{args.w_contrastive}",
            f"f-text_{args.w_fare_text}",
            f"f-img_{args.w_fare_image}"
        ])
    ...
    """
    if args.w_contrastive == 0 and args.w_fare_text == 0:
        for param in model.transformer.parameters():
            param.requires_grad_(False)
    if args.w_contrastive == 0 and args.w_fare_image == 0:
        for param in model.visual.parameters():
            param.requires_grad_(False)

    completed_epoch = 0
    if any(v in data for v in ('val', 'imagenet-val', 'imagenet-v2')):
        evaluate(model, model_frozen, preprocess_val_without_normalize, normalize, data, completed_epoch, args, tb_writer=writer, tokenizer=tokenizer)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_one_epoch(model, model_frozen, normalize, tokenizer, V, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=writer)
