...
def zero_shot_classifier(model, tokenizer, classnames, templates, device, amp=True):
    ...
    with torch.no_grad(), autocast():
        ...
        for classname in tqdm(classnames):
            ...
            if isinstance(model, CLIP):  # open_clip
                class_embeddings = model.encode_text(texts, normalize=True)
            elif isinstance(model, CLIPModel):  # hf clip
...
def run_classification(model, classifier, dataloader, device, normalize=None, resize=None, amp=True,
    ...
    def _forward_unnorm(data_unnorm):
        ...
        if isinstance(model, CLIP):  # open_clip
            features = model.encode_image(data_norm, normalize=True)
        elif isinstance(model, CLIPModel):  # hf clip
            features = model.get_image_features(data_norm)
