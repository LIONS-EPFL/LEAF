...
device_id = GPUtil.getFirstAvailable(
        order="memory", maxLoad=1.0, maxMemory=0.1, attempts=5, interval=10,
        # excludeID=[0, 1, 2, 3, 4, 5],
        verbose=False
)
os.environ["CUDA_VISIBLE_DEVICES"] = str(device_id[0])
...
from clip_benchmark.model_loading import load_clip, MODEL_TYPES
...
def get_parser_args():
    ...
    parser_eval.add_argument('--num_workers', default=1, type=int)
    ...
    parser_eval.add_argument('--model_type', default="hf_clip", type=str, choices=MODEL_TYPES, help="clip model type")
...
def main_eval(base):
    ...
    for model, pretrained in models:
        print(f"[Model] {model} [Pretrained] {pretrained}")
...
def get_model_shortname(pretrained):
    if os.path.isfile(pretrained):
        if "checkpoints" in pretrained:
            return pretrained.split('/')[-3]
        else:
            return pretrained.split('/')[-1]
    else:
        return pretrained
...
def run(args):
    ...
    pretrained_slug = get_model_shortname(args.pretrained)
    ...
    output = args.output.format(
        model=args.model.replace('hf-hub:laion/', ''),
        attack=args.attack,
    ...
    if args.verbose:
        ...
        print(f"[model] {args.model} [pretrained] {args.pretrained} [language] {args.language}")
    ...
    if args.skip_load:
        ...
        if args.n_samples > 0:
            assert args.num_workers <= 1, "otherwise we get different samples"
            rng = random.Random(args.seed)
            # deterministic shuffle and subset
            dataset = dataset.shuffle(10_000, initial=10_000, rng=rng).slice(args.n_samples)
    ...
    if task == "zeroshot_classification":
        ...
        from clip_benchmark.metrics import captioning
