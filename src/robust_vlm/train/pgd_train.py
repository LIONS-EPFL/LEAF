...
from robust_vlm.train.utils import project_perturbation, normalize_grad
...
def pgd(
    ...
    for i in range(iterations):
        ...
        with torch.enable_grad():
            data_adv = data_clean + perturbation
            out = forward(data_adv, output_normalize=output_normalize)
        ...
        with torch.no_grad():
            ...
            assert torch.max(data_clean + perturbation) < 1.0 + 1e-6 and torch.min(data_clean + perturbation) > -1e-6
