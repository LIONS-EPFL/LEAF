...
def train_one_epoch(model, model_frozen, normalize, tokenizer, V, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=None):
    device = torch.device(args.device)
    autocast = get_autocast(args.precision)
    input_dtype = get_input_dtype(args.precision)
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            text_features_frozen = model_frozen.encode_text(tokenizer(texts).to(device=device, non_blocking=True), normalize=args.normalize_fare)
            image_features_frozen = model_frozen.encode_image(normalize(images), normalize=args.normalize_fare)
            if args.attack_objective == 'contrastive':
                image_features = model.encode_image(image=normalize(images),normalize = True).view(len(texts),1,-1)
                adv_text_features, adv_texts = attack_text(model,tokenizer,texts,image_features,device,objective='dissim',n=args.n_charmer,k=args.k_adv,V=V,debug=True)
            elif args.attack_objective == 'fare':
                adv_text_features, adv_texts = attack_text(model,tokenizer,texts,text_features_frozen,device,objective='l2',n=args.n_charmer,k=args.k_adv,V=V,debug=True)        

        adv_texts = tokenizer(adv_texts).to(device=device, non_blocking=True)

        if args.attack_objective == 'contrastive':
            image_features = model.encode_texts(adv_texts,normalize = True).view(len(texts),1,-1)
            adv_images = attack_image(model,normalize,images.detach(),adv_text_features,device,objective='dissim',eps=args.eps_adv,n_steps=args.n_steps_adv,stepsize=args.stepsize_adv,debug=False)
        if args.attack_objective == 'fare':
            adv_images = attack_image(model,normalize,images.detach(),image_features_frozen,device,objective='l2',eps=args.eps_adv,n_steps=args.n_steps_adv,stepsize=args.stepsize_adv,debug=False)
        ...
        with autocast():
            text_features_adv = model.encode_text(adv_texts, normalize=args.normalize_fare)
            image_features_adv = model.encode_image(normalize(adv_images), normalize=args.normalize_fare)

            logit_scale = model.logit_scale.exp()

            model_out = {
                "image_features": image_features_adv/image_features_adv.norm(dim=-1, keepdim=True),
                "text_features": text_features_adv/text_features_adv.norm(dim=-1, keepdim=True),
                "logit_scale": logit_scale
            }
            if args.distill:
                with torch.no_grad():
                    dist_model_out = dist_model(normalize(adv_images), adv_texts)
        ...
        with autocast():    
            ...
            loss_FARE_image = F.mse_loss(image_features_frozen,
                                            image_features_adv,reduction='none').sum(dim=-1).mean()


        '''
        TOTAL loss:
        '''
        total_loss = (args.w_contrastive*sum(losses.values()) + args.w_fare_text*loss_FARE_text + args.w_fare_image*loss_FARE_image) / args.accum_freq
        if 'loss_contrastive' not in losses_accum:
            losses_accum['loss_contrastive'] = sum(losses.values()) / args.accum_freq
            losses_accum["loss"] = total_loss / args.accum_freq
            losses_accum['loss_FARE_text'] = loss_FARE_text / args.accum_freq
            losses_accum['loss_FARE_image'] = loss_FARE_image / args.accum_freq
        else:
            losses_accum['loss_contrastive'] += sum(losses.values()) / args.accum_freq
            losses_accum["loss"] += total_loss / args.accum_freq
            losses_accum['loss_FARE_text'] += loss_FARE_text / args.accum_freq
